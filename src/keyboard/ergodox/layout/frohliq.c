/* ----------------------------------------------------------------------------
 * ergoDOX layout : Frohliq (modified from the 'Qwerty' layout, yet designed for
 * the sake of software-mapping to Dvorak.)
 * ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
// Notes on the current layout and (potential) future layout modifications
//
//      - Perhaps swap the location of the GUI keys (i.e. command) and the
//      control keys, which are currently a bit of an awkward reach. If I'm
//      using this keyboard mostly within Vim, then the GUI keys are next to
//      useless! Although, to be honest, I *do* use the command keys a lot,
//      since I *am* a Mac user. :P
//
//      - Make the key repeat trigger sooner; this is especially annoying for
//      the delete key right now. This has do do with Karabiner, which should be
//      modified to have two separate settings for internal and external
//      keyboards, which aren't compatible right now.
//
//      - This may be more on the software side, but parentheses, brackets,
//      braces, etc. *need* a more accesible spot!
//
//      - Speaking of accesible spots, I think the numbers should be moved to
//      the row below the home row when the '2nd left thumb modifier' is
//      pressed.
// ----------------------------------------------------------------------------
...
0,
// left hand
  _bracketR,     _1,         _2,      _3,      _4, _5,  _esc,
       _esc,     _Q,         _W,      _E,      _R, _T, _dash,
       _tab,     _A,         _S,      _D,      _F, _G,
      _guiL,     _Z,         _X,      _C,      _V, _B,     1,
      _guiL, _grave, _backslash, _arrowL,  _ctrlL,
                                                        _guiL,  _altL,
                                                    0,      0,  _home,
                                              _shiftL,  _altL,   _end,
// right hand
           ...
           _equal, _Y,      _U,       _I,      _O,         _P, _bracketL,
                ...
                1, _N,      _M,   _comma, _period,     _slash,     _guiR,
                         _ctrlR, _arrowD, _arrowU,    _arrowR,     _guiR,
 _altR, _guiR,
_pageU,      0,      0,
...
// left hand
 ...
 ktrans,sshprre,sshprre, kprrel, kprrel,   NULL, kprrel,
